...
/* 関数定義main()関数以降に記述 */
int ProcessFrame(int, char*, char*);
...
int main(){
	...
	/* choose run1 or run2 */
	printf("run1 ,run2 or other \nenter 1,2,an integer:");
	scanf("%d",&basetype);
	...
	for(loop=12; loop>1; loop--){
		...
		result = ProcessFrame(basetype, fname[selfile], linebuffer);
		if(result == 1){
	...
	/* 処理終了を告げる */
	printf("\n*** End distortion-i.exe ... ***\n");
	
	/* main() 戻り値 0（正常終了） */
	return 0;
}

int ProcessFrame(int basetype, char *fname, char *path){
	int loop;	/* ループカウンタ */
	int line;	/* 読み取り行数 */
	...
	while(fgets(linebuffer,sizeof(linebuffer),read)!=NULL){
		...
		while(pos < 3.0){
			...
			/* 上流乳剤とベースの境界座標を取得 */
				...
				/* 表面認識 */
				SCheck[1] += 1;
				if(SCheck[1]==1)base[0]=loop;
				if(SCheck[1]>1)loop--;//donot memorize
			}
			...
			/* 下流乳剤とベースの境界座標を取得 */
				...
				/* 表面認識 */
				SCheck[2] += 1;
				if(SCheck[2]==1)base[1]=loop;
				if(SCheck[2]>1)loop--;//donot memorize
			}
		...
		for(loop=0; loop<Max; loop++)
			...
			/* Z方向への折り返し(5micrion over)があれば通知 */
			if( (fz[loop]+0.005) > fz[loop-1])
				printf("  Error in tr#%d - pl#%d, -> [fz[%d]=%.5f] is much bigger than [fz[%d]=%.5f] !!\n", tr.curr, pl, loop, fz[loop], loop-1, fz[loop-1]);
		
		/* Error Check */
//		for(loop=0; loop<4; loop++)

			/* ２度以上同じ乾板表面がある場合も通知 */
			if((SCheck[0] > 1) || (SCheck[3] > 1))
				printf("  Error in Surface Identification -> tr#%d - pl#%d !!\n", tr.curr, pl);

			/* mean of base position */
			for(loop=0;loop<3;loop++){
				surface[loop][1]=surface[loop][1]/((double)SCheck[1]);
				surface[loop][2]=surface[loop][2]/((double)SCheck[2]);
			}

			/* return mean */
			fx[base[0]]=surface[0][1];
			fy[base[0]]=surface[1][1];
			fz[base[0]]=surface[2][1];

			fx[base[1]]=surface[0][2];
			fy[base[1]]=surface[1][2];
			fz[base[1]]=surface[2][2];
		...
		SFd = 0.5/ThickEMd;
		/* Fix BaseValue */
		if(basetype==1)BaseValue = 0.050;/* run1 */
		else if(basetype==2)BaseValue = 0.040;/* run2 */
		else BaseValue = sqrt((surface[2][1]-surface[2][2])*(surface[2][1]-surface[2][2]));
		/* Base track初期値 */
		Bx0 = surface[0][3];
		By0 = surface[1][3];
