...
int main(int argc, char *para[])
{
	...
	if ((smp = fopen("sample_tr.dlt", "w")) == NULL) {
		printf("output_file[sample_tr.dlt] not open\n");
		exit(1);
	}
	
	/* Log fileを作成 */
	if ((log = fopen("delta.log", "w")) == NULL) {
		printf("output_file[delta.log] not open\n");
		exit(1);
	}
	...
	while(fgets(linebuffer,sizeof(linebuffer),read) != NULL){
		...
		while(1){
			...
			if(pljoint[1] == 0 && pljoint[2] == 0){
				...
				/* Sample Fileに出力 */
				fprintf(smp,"%s\t%lf\t%lf\t%lf\t%lf\t000\n",ID,R[2],Diff.x,Diff.y,Diff.z);
